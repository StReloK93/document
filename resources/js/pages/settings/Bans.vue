<template>
    <section class="flex h-full">
        <aside class="h-full w-2/4 flex flex-col">
...
const columns = reactive([
    ...
    { field: "description", headerName: 'Sabab', flex: 1 , editable: true},
    { 
        headerName: '',
        width: 60,
...
axios.get('negationslist').then(({ data }) => types.value = data)


function storeType() {
    axios.post('negationslist', { name: 'nomlang' }).then(({ data }) => {
...
function cellEdit(cell) {
    ...
    axios.patch(`negationslist/${cell.data.id}`, { name: cell.value }).then(() => {
...
function deleteType(cell){
    swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(`negationslist/${cell.data.id}`).then(() => {
